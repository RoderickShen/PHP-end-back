...
class Index
{
    ...
    public function showComment(Request $request){
        ...
        if($request->isGet()){
            ...
            if(!$isUser){
                ...
                $res= Comments::where("openid","eq",$openid)->select();
            }
        }
        return json($res);
    }
    ...
    public function getInformation(Request $request){
        ...
        if($request->isGet()){
            ...
            if(!$isUser){
                $bookid= request()->get("bookid");
                $res= Book::where("bookid","eq",$bookid)->select();
            } else {
                $openid=request()->get("openid");
    ...
    public function addShoppingcar(Request $request) {
        ...
        if($request->isPost()){
          $openid=$request->post('openid');
          $bookid=$request->post('bookid');
          if(Shoppingcar::where("openid","eq",$openid)->where("bookid","eq",$bookid)->count()){
              $res=-1;
          } else {
              Shoppingcar::create(['bookid'=>$bookid,'openid' => $openid]);
              $res=1;
          }
       }
       return json(["msg"=>$res]);
